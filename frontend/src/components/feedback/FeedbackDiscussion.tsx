...
export function FeedbackDiscussion({
  ...
  return (
    <div className="bg-white rounded-md shadow-sm border border-gray-100 flex flex-col h-[600px]">
      <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50 rounded-t-xl">
        <h2 className="font-bold text-gray-900 flex items-center gap-2">
          <div className="p-1.5 bg-blue-100 text-[#1a4f9c] rounded-lg">
        ...
        {commentsQuery.isLoading ? (
          ...
          <div className="text-center py-12 text-red-600 space-y-2">
            ...
            <button
              ...
              className="px-3 py-1.5 text-sm font-bold bg-[#1a4f9c] text-white rounded-lg hover:bg-[#1a4f9c] transition-colors"
            >
              Retry
            </button>
          </div>
          ...
          comments.map((entry: any) => {
            ...
            return (
              <div key={entry.id} className={`flex gap-3 ${isMe ? 'flex-row-reverse' : ''}`}>
                <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shadow-sm ${isMe ? 'bg-[#1a4f9c] text-white' : 'bg-white border border-gray-200 text-gray-700'
                  }`}>
                ...
                <div className={`flex flex-col max-w-[80%] ${isMe ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`p-3 rounded-md text-sm shadow-sm relative ${isMe
        ...
        <div className="p-4 bg-white border-t border-gray-100 rounded-b-xl">
          {isHr && (
            <div className="flex items-center justify-between mb-3">
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                />
                <span className="text-xs font-bold text-gray-600">Internal Note (Hidden from employee)</span>
              </label>
              <button
                type="button"
                onClick={handleGenerateAIReply}
                disabled={isGeneratingAI}
                className="text-xs flex items-center gap-1 text-purple-600 hover:text-purple-700 font-bold transition-colors"
              >
          ...
          <div className="relative">
            <textarea
              ...
              className={`w-full pl-4 pr-12 py-3 rounded-md border focus:outline-none focus:ring-2 transition-all resize-none ${isInternal
            ...
            <div className="absolute bottom-2 right-2 flex items-center gap-2">
              ...
              <button
                ...
                className={`p-2 rounded-full text-white transition-all shadow-sm ${(!comment.trim() && commentAttachmentIds.length === 0) || addCommentMutation.isPending
                  ...
                  : 'bg-[#1a4f9c] hover:bg-[#1a4f9c] hover:shadow-md hover:scale-105'
                  }`}
              >
