  ...
  return (
    <div className="bg-white rounded-md shadow-sm border border-gray-100 p-5 space-y-4">
      ...
      <div className="space-y-3">
        <div>
          <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Current Status</p>
          {isHr ? (
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-3 py-2 bg-[#1a4f9c] text-white text-sm rounded-lg hover:bg-[#1a4f9c] disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {updateStatusMutation.isPending ? 'Saving...' : 'Save'}
              </button>
            </div>
          ) : (
            <span className={`inline-block px-3 py-1.5 text-sm font-bold rounded-lg border ${statusBadgeMap[feedback.status]}`}>
        ...
        <div>
          <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Assigned To</p>
        ...
        {(feedback as any).sla_status && (
          <div className="pt-3 border-t border-gray-100">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">SLA Status</p>
            <div className={`p-3 rounded-lg border ${(feedback as any).sla_status === 'BREACHED'
                ? 'bg-red-50 border-red-100'
                : 'bg-blue-50 border-blue-100'
              }`}>
              <div className="flex items-center gap-2 mb-1">
                <Clock className={`h-4 w-4 ${(feedback as any).sla_status === 'BREACHED' ? 'text-red-600' : 'text-[#1a4f9c]'}`} />
                <span className={`text-sm font-bold ${(feedback as any).sla_status === 'BREACHED' ? 'text-red-700' : 'text-[#1a4f9c]'}`}>
              ...
              {(feedback as any).sla_status && (
                <span className={`inline-block mt-2 px-2.5 py-1 text-[11px] font-bold rounded-full border ${getSlaBadgeStyles((feedback as any).sla_status)}`}>
