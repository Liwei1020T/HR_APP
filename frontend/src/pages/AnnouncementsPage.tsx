...
const CATEGORY_CONFIG: Record<string, { label: string; icon: React.ElementType; color: string; bgColor: string }> = {
  COMPANY_NEWS: { label: 'Company News', icon: Megaphone, color: 'text-[#1a4f9c]', bgColor: 'bg-blue-100' },
  ...
  const AnnouncementCard = ({ announcement, highlighted = false }: { announcement: Announcement; highlighted?: boolean }) => {
    ...
    return (
      <div
        ...
        className={`group relative overflow-hidden rounded-md transition-all duration-300 hover:-translate-y-1 hover:shadow-sm border border-gray-200 focus:outline-none ${highlighted
          ? 'bg-amber-50 border border-amber-100 shadow-md'
      ...
      >
        ...
        <div className="p-6">
          <div className="flex items-start gap-4">
            <div className={`p-3 rounded-md ${meta.bgColor} ${meta.color} shadow-sm group-  transition-transform duration-300`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center justify-between pt-4 border-t border-gray-100/50">
                ...
                {announcement.attachments && announcement.attachments.length > 0 && (
                  <span className="text-xs font-bold text-[#1a4f9c] flex items-center gap-1 bg-blue-50 px-2 py-1 rounded-md">
  ...
  return (
    <AppLayout>
      ...
      <div className="space-y-8">
          ...
          <div className="relative flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            ...
            {canManage && (
              <button
                ...
                className="group flex items-center justify-center gap-2 px-6 py-3 bg-white text-[#1a4f9c] rounded-md font-bold hover:bg-blue-50 transition-all shadow-sm border border-gray-200 hover:shadow-md border border-gray-200 transform hover:-translate-y-0.5"
              >
          ...
          {categoryOptions.map((category) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2.5 rounded-md text-sm font-bold transition-all whitespace-nowrap ${isSelected
        ...
        {isLoading ? (
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="h-64 bg-white rounded-md shadow-sm animate-pulse border border-gray-100"></div>
            ))}
          </div>
        ) : announcements.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 bg-white rounded-md border border-gray-100 border-dashed">
      ...
      {showCreateModal && canManage && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div className="fixed inset-0 bg-gray-900/75   transition-opacity" onClick={() => setShowCreateModal(false)}></div>

            <div className="relative transform overflow-hidden rounded-md bg-white text-left shadow-md transition-all sm:my-8 sm:w-full sm:max-w-2xl">
              <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4">
                ...
                <form onSubmit={handleCreateSubmit} className="space-y-5">
                  ...
                  <div>
                    <label className="block text-sm font-bold text-gray-700 mb-1.5">Attachments</label>
                    <div
                      onClick={() => attachmentInputRef.current?.click()}
                      className="border-2 border-dashed border-gray-300 rounded-md p-6 text-center hover:border-blue-500 hover:bg-blue-50 transition-colors cursor-pointer"
                    >
                      <input
                      ...
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <p className="text-sm text-gray-600">
                          <span className="font-bold text-[#1a4f9c]">Click to upload</span> or drag and drop
                        </p>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    ...
                    <div className="flex items-center pt-6">
                      <label className="flex items-center gap-3 cursor-pointer group">
                        ...
                        <span className="text-sm font-bold text-gray-700 group-hover:text-gray-900">Pin to top</span>
                      </label>
                    </div>
                  </div>
      ...
      {selectedAnnouncement && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            <div className="fixed inset-0 bg-gray-900/75   transition-opacity" onClick={() => setSelectedAnnouncement(null)}></div>

            <div className="relative transform overflow-hidden rounded-md bg-white text-left shadow-md transition-all sm:my-8 sm:w-full sm:max-w-3xl border border-gray-100">
              {/* Modal Header */}
              <div className={`relative px-6 py-8 ${getCategoryMeta(selectedAnnouncement.category).bgColor} overflow-hidden`}>
                ...
                <div className="relative flex justify-between items-start z-10">
                  <div className="flex items-start gap-5">
                    <div className={`p-3.5 rounded-md bg-white shadow-sm ${getCategoryMeta(selectedAnnouncement.category).color}`}>
                      {(() => {
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-3 mb-3">
                        <span className={`px-3 py-1 rounded-full text-xs font-bold tracking-wide uppercase bg-white/60   ${getCategoryMeta(selectedAnnouncement.category).color}`}>
              ...
              <div className="px-8 py-6">
                ...
                <div className="flex items-center gap-6 text-sm text-gray-500 pb-6 border-b border-gray-100 mb-6">
                  <div className="flex items-center gap-2">
                    <div className="h-8 w-8 rounded-full bg-[#1a4f9c] flex items-center justify-center text-white font-bold shadow-sm">
                      <User className="h-4 w-4" />
                    </div>
                    <span className="font-bold text-gray-900">{formatCreatorName(selectedAnnouncement)}</span>
                  </div>
                ...
                {selectedAnnouncement.attachments && selectedAnnouncement.attachments.length > 0 && (
                  <div className="mb-8">
                    <h4 className="text-sm font-bold text-gray-900 uppercase tracking-wider mb-4 flex items-center gap-2">
                      <FileText className="h-4 w-4 text-[#1a4f9c]" />
                ...
                <div className="flex items-center justify-between pt-6 border-t border-gray-100">
                  {canManage ? (
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2.5 rounded-md text-sm font-bold transition-all shadow-sm border ${selectedAnnouncement.is_pinned
                      ...
                      <button
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 bg-white text-red-600 border border-red-200 rounded-md text-sm font-bold hover:bg-red-50 hover:border-red-300 transition-all shadow-sm"
                      >
                  ...
                  <button
                    onClick={() => setSelectedAnnouncement(null)}
                    className="px-6 py-2.5 bg-gray-900 text-white rounded-md text-sm font-bold hover:bg-gray-800 transition-all shadow-md hover:shadow-sm border border-gray-200 transform hover:-translate-y-0.5"
                  >
                    Close
                  </button>
                </div>
